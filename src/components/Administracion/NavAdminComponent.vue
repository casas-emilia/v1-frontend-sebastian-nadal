<template>
  ...
  onMounted(async () => {
    ...
    if (authStore) {
      try {
        ...
        const response = await axios.get(`/administracion/empresas/1/usuarios/${authStore.userId}`);
  ...
  const logout = () => {
    ...
    // Llamar a la función logout del authStore
    authStore.logout();

    // Redirigir a login y mostrar el mensaje de SweetAlert
    router.push('/login');
    Swal.fire({
      icon: 'success',
